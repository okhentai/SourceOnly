...
void AWeapon::OnSphereOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	{
		if (WeaponType == EWeaponType::EWT_Flag && StarRailCharacter->GetTeam() == Team) return;
		if (StarRailCharacter->IsHoldingTheFlag()) return;
...
void AWeapon::ClientAddAmmo_Implementation(int32 AmmoToAdd)
{
	...
	Ammo = FMath::Clamp(Ammo + AmmoToAdd, 0, MagCapacity);
	StarRailOwnerCharacter = StarRailOwnerCharacter == nullptr ? Cast<AStarRailCharacter>(GetOwner()) : StarRailOwnerCharacter;
	if (StarRailOwnerCharacter && StarRailOwnerCharacter->GetCombat() && IsFull())
	{
		StarRailOwnerCharacter->GetCombat()->JumpToShotgunEnd();
	}
	SetHUDAmmo();
}
...
void AWeapon::OnRep_Owner()
{
	...
	{
		StarRailOwnerCharacter = StarRailOwnerCharacter == nullptr ? Cast<AStarRailCharacter>(Owner) : StarRailOwnerCharacter;
		if (StarRailOwnerCharacter && StarRailOwnerCharacter->GetEquippedWeapon() && StarRailOwnerCharacter->GetEquippedWeapon() == this)
		{
			SetHUDAmmo();
		}
	}
...
void AWeapon::OnEquipped()
{
	...
	StarRailOwnerCharacter = StarRailOwnerCharacter == nullptr ? Cast<AStarRailCharacter>(GetOwner()) : StarRailOwnerCharacter;
	if (StarRailOwnerCharacter && bUseServerSideRewind)
	{
		StarRailOwnerController = StarRailOwnerController == nullptr ? Cast<AStarRailPlayerController>(StarRailOwnerCharacter->Controller) : StarRailOwnerController;
		if (StarRailOwnerController && HasAuthority() && !StarRailOwnerController->HighPingDelegate.IsBound())
		{
			StarRailOwnerController->HighPingDelegate.AddDynamic(this, &AWeapon::OnPingTooHigh);
		}
	}
}
...
void AWeapon::OnDropped()
{
	...
	if (StarRailOwnerCharacter)
	{
		...
		if (StarRailOwnerController && HasAuthority() && StarRailOwnerController->HighPingDelegate.IsBound())
		{
			StarRailOwnerController->HighPingDelegate.RemoveDynamic(this, &AWeapon::OnPingTooHigh);
		}
	}
}
